...
#define APPNAME          wstr("Custom CheckBox")
#define APPCLASSNAME     wstr("custom_checkbox_class")
...
dim shared as HWND ghCheck(0 to CHECKBOX_COUNT - 1)
...
function WinMain( _
    ...
    ' Load the Segoe Fluent Icons ttf file that supplies the demo's box glyphs. The CONTROL has
    ' no opinion about which font its glyphs come from -- it just draws the string it is given
    ' with the HFONT it is handed -- so this is the demo's business, not the control's.
