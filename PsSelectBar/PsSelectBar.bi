...
' as it always has; a host opts an instance into PsTooltip with PsSelectBar_SetTooltipMode.
...
type PSSELECTBAR
    ...
    ' pair; PsSelectBar_GetTooltipHandle still answers the comctl32 handle, and 0 while
...
declare function PsSelectBar_SetTooltipMode( byval hSelectBar as HWND, byval nMode as long ) as boolean
declare function PsSelectBar_GetTooltipMode( byval hSelectBar as HWND ) as long
...
declare function PsSelectBar_GetPsTooltipHandle( byval hSelectBar as HWND ) as HWND
' Honoured by BOTH backends. A delay never set keeps the backend's own derivation from
' the system double-click time.
declare sub      PsSelectBar_SetAutoPopTime( byval hSelectBar as HWND, byval milliseconds as long )
declare sub      PsSelectBar_SetReshowTime( byval hSelectBar as HWND, byval milliseconds as long )
