...
#define APPNAME          wstr("Custom OptionButton")
#define APPCLASSNAME     wstr("custom_optionbutton_class")

#DEFINE GUIFONT          wstr("Segoe UI")

' NO SYMBOLFONT, AND NO SegoeFluentIcons.ttf IN THIS REPO. PsCheckBox needs an icon font because
' its box is a composite GLYPH; this control draws its circle as geometry through GDI+, so there
' is no font to load, no AddFontResourceEx to mirror on exit, and no missing-glyph failure mode
...
dim shared as HWND ghOpt(0 to OPTION_COUNT - 1)
...
function WinMain( _
    ...
    ' Initialize GDI+ (PsBufferPaint draws all geometry through it, and this control's circle IS
    ' geometry). Must be running before the first WM_PAINT builds a buffer, and must outlive
    ' every one of them, so it brackets frmMain_Show.
