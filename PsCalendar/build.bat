@echo off
rem ========================================================================================
rem  PsCalendar demo build script.
rem
rem  Usage:  build.bat          builds main.exe
rem          build.bat clean    deletes build output
rem
rem  Run the geometry / date-math self-test with:  set PSCALENDAR_SELFTEST=1 && main.exe
rem  Dump computed geometry with:                  set PSCALENDAR_TRACE=1 && main.exe
rem ========================================================================================
...
rem measured in this control, all of which derive from DPI-scaled metrics and a scaled font.
...
%FBC% -i %INCLUDE_ROOT% -w all main.bas %RESOURCE%
if errorlevel 1 (
    echo.
    echo BUILD FAILED.
    exit /b 1
