...
#define APPNAME          wstr("Custom Calendar")
#define APPCLASSNAME     wstr("custom_calendar_class")
...
dim shared as HWND ghCal(0 to CAL_COUNT - 1)
dim shared as HWND ghBtnBefore, ghBtnAfter
...
' PsCalendar wraps no child control and owns no popup, so the include list is the shortest in
' the family: the paint surface and the control itself. Nothing else.
